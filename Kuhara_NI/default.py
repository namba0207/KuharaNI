...
# ----------------------------
DEVICE_NAME = "Dev1"
CHANNELS = ["ai7", "ai17", "ai0", "ai1", "ai2", "ai3", "ai4", "ai5"]
SAMPLING_RATE = 10000
BUFFER_SIZE = 50
    ...
    def update_plot(self):
        try:
            while not self.data_queue.empty():
                acc, acc2, fx, fy, fz, mx, my, mz = self.data_queue.get_nowait()
        ...
        with nidaqmx.Task() as task:
            ...
            while True:
                ...
                for i in range(BUFFER_SIZE):
                    ...
                    self.data_queue.put((acc1_v,acc2_v, fx, fy, fz, mx, my, mz))  # プロットに与えるデータ（ここをいじることで、描画するデータを変化できる）

                    if self.recording and self.record_start_time is not None:
                        rel_time = float(self.sample_counter) / SAMPLING_RATE
                        try:
                            self.record_queue.put((rel_time, acc1_v, acc2_v, fx, fy, fz, mx, my, mz)) #記録されるデータのやり取り。今回は加速度2ch、６軸センサ、時間の9ch
